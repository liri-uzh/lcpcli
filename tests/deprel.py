...
def create_corpus():
    ...
    s1 = c.Segment(t1, t2)
    s1.make()
    c.DepRel(head=t1, dependent=t2, udep="obj").make()
    c.DepRel(dependent=t1, udep="root").make()
    t3 = c.Token("bye")
    t4 = c.Token("world")
    s2 = c.Segment(t3, t4)
    s2.make()
    c.DepRel(head=t3, dependent=t4, udep="obj").make()
    c.DepRel(dependent=t3, udep="root").make()
